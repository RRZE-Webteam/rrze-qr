...
function add_action($hook, $callback, $priority = 10, $accepted_args = 1) { $GLOBALS['hooks'][$hook][$priority][] = [$callback, $accepted_args]; }
function add_filter($hook, $callback, $priority = 10, $accepted_args = 1) { add_action($hook, $callback, $priority, $accepted_args); }
function plugin_hook($hook, $priority = 10): callable {
    $entries = $GLOBALS['hooks'][$hook][$priority] ?? [];
    check(count($entries) === 1, "Expected one callback for $hook at priority $priority");
    check(is_callable($entries[0][0]), "Callback for $hook must be callable");
    return $entries[0][0];
}
...
foreach (['Utilities', 'Settings', 'QRCode', 'Main'] as $class) { require __DIR__ . '/../includes/' . $class . '.php'; }
...
$main->onLoaded();
$enqueue = plugin_hook('admin_enqueue_scripts');
$addLink = plugin_hook('post_row_actions');
$pageLink = plugin_hook('page_row_actions');
$registerMenu = plugin_hook('admin_menu');
$migrate = plugin_hook('admin_init', 5);
$redirect = plugin_hook('admin_init');
plugin_hook('wp_ajax_rrze_qr_save_defaults');
check(count($GLOBALS['hooks']) === 6, 'All plugin hooks must be registered');
check($GLOBALS['hooks']['post_row_actions'][10][0][1] === 2 && $GLOBALS['hooks']['page_row_actions'][10][0][1] === 2, 'Row actions need both actions and post arguments');
$registerMenu();
$render = $GLOBALS['menu'][4];
check(is_callable($render), 'Registered menu renderer must be callable');
check($pageLink([], $GLOBALS['posts'][42]) === $addLink([], $GLOBALS['posts'][42]), 'Post and page hooks must expose the same action');
echo "PHP hook registration checks passed.\n";
function save_defaults($data): JsonResponse {
    $_POST = $data;
    try { plugin_hook('wp_ajax_rrze_qr_save_defaults')(); } catch (JsonResponse $response) { return $response; }
...
foreach (['white', 'black', 'fau'] as $fg) {
    foreach (['white', 'black', 'fau', 'transparent'] as $bg) {
        ...
        $response = save_defaults(['foreground' => $fg, 'background' => $bg, 'size' => '600']);
...
foreach ([[], ['foreground' => ['black'], 'background' => 'white', 'size' => '300'], ['foreground' => 'red', 'background' => 'white', 'size' => '300'], ['foreground' => 'black', 'background' => 'white', 'size' => '99999']] as $invalid) {
    check(save_defaults($invalid)->status === 400, 'Malformed defaults must be rejected');
}
$response = save_defaults(['foreground' => ' #A1b ', 'background' => '#FEDcBa', 'size' => '600']);
...
foreach (['#12', '#gggggg', '#0008', '#00000080', 'url(x)', 'rgb(0,0,0)', ['#123456']] as $invalid) {
    foreach (['foreground', 'background'] as $field) {
        ...
        $input[$field] = $invalid;
        check(save_defaults($input)->status === 400, 'Invalid custom colors must be rejected');
        check($GLOBALS['options']['rrze_qr_defaults'] === $custom, 'Invalid custom colors must leave defaults unchanged');
    }
}
check(save_defaults(['foreground' => '#fff', 'background' => 'white', 'size' => '300'])->status === 400, 'Equivalent colors must be rejected after normalization');
check(save_defaults(['foreground' => 'transparent', 'background' => '#fff', 'size' => '300'])->status === 400, 'Foreground must be opaque');
check(save_defaults(['foreground' => '#123456', 'background' => 'transparent', 'size' => '300'])->success, 'Custom foreground supports transparency');
$GLOBALS['caps'] = ['edit_posts' => true];
check(save_defaults(['foreground' => 'black', 'background' => 'white', 'size' => '300'])->status === 403, 'Editors must not save site defaults');
$GLOBALS['caps'] = ['manage_options' => true];
$GLOBALS['valid_nonce'] = false;
check(save_defaults(['foreground' => 'black', 'background' => 'white', 'size' => '300'])->status === 403, 'Saving requires a valid nonce');
$GLOBALS['valid_nonce'] = true;
$GLOBALS['fail_update'] = true;
check(save_defaults(['foreground' => 'black', 'background' => 'white', 'size' => '1200'])->status === 500, 'Storage failures must be reported');
$GLOBALS['fail_update'] = false;
check(save_defaults(['foreground' => 'black', 'background' => 'white', 'size' => '1200'])->success, 'Administrator can save defaults');
echo "PHP defaults checks passed.\n";

$GLOBALS['caps'] = [];
try { $render(); throw new RuntimeException('Unauthorized page access succeeded'); }
...
foreach (['edit_posts', 'edit_pages', 'manage_options'] as $capability) {
    ...
    ob_start(); $render(); $html = ob_get_clean();
    check(str_starts_with($html, '<div class="wrap rrze-qr-workspace">') && str_contains($html, '<h1>QR-Codes</h1>'), 'Translated text must remain inside the markup');
    check(str_contains($html, 'id="rrze-qr-app"'), 'Workspace must render the React mount point');
}
$GLOBALS['pagenow'] = 'tools.php';
$_GET['page'] = 'rrze-qr';
try { $redirect(); throw new RuntimeException('Legacy page was not redirected'); }
...
$actions = $addLink([], $GLOBALS['posts'][42]);
...
check($addLink([], $GLOBALS['posts'][42]) === [], 'Unsupported types must have no row action');
try { $render(); throw new RuntimeException('Unsupported context succeeded'); }
...
foreach (['', '0', '-1', '42x', '1.5', ['42'], str_repeat('9', 40)] as $invalid) {
    ...
    try { $enqueue('toplevel_page_rrze-qr'); throw new RuntimeException('Invalid context succeeded'); }
...
foreach (['draft', 'private', 'trash', 'future'] as $status) {
    ...
    check($addLink([], $GLOBALS['posts'][42]) === [], 'Non-public posts must have no row action');
    try { $enqueue('toplevel_page_rrze-qr'); throw new RuntimeException('Non-public context succeeded'); }
...
foreach (['99', '42'] as $id) {
    ...
    try { $render(); throw new RuntimeException('Unauthorized context succeeded'); }
...
foreach ([
    ...
    $GLOBALS['allowed'] = true;
    check(count($addLink([], $GLOBALS['posts'][42])) === 1, "$role must be able to create QR codes for editable posts");
    $_GET['post_id'] = '42';
    $enqueue('toplevel_page_rrze-qr');
    ...
    check(save_defaults(['foreground' => 'black', 'background' => 'white', 'size' => '300'])->status === 403, "$role must not save site defaults");
    $GLOBALS['allowed'] = false;
    check($addLink([], $GLOBALS['posts'][42]) === [], 'Non-editable posts must not have a QR action');
    try { $render(); throw new RuntimeException('Non-editable post context succeeded'); }
...
check($addLink([], $GLOBALS['posts'][42]) === [], 'Subscribers must not have a QR action');
try { $render(); throw new RuntimeException('Subscriber access succeeded'); }
...
foreach (['128', '512', '0750', '4096'] as $size) {
    $response = save_defaults(['foreground' => 'black', 'background' => 'white', 'size' => $size]);
...
foreach ([null, '', ' ', '0', '-1', '127', '4097', '512.5', '1e3', '512px', ' 512', ['512'], true, 512.5] as $size) {
    check(save_defaults(['foreground' => 'black', 'background' => 'white', 'size' => $size])->status === 400, 'Malformed or out-of-range sizes must be rejected');
...
// Exercise the registered migration, including repeat calls and legacy defaults.
$GLOBALS['options'] = ['rrze_qr_color' => 'fau_on_transparent'];
$migrate();
$settings = new RRZE\QR\Settings();
check($settings->getDefaults() === ['foreground' => '#04316a', 'background' => 'transparent', 'size' => 300], 'Migration must retain legacy colors and transparency');
check(!isset($GLOBALS['options']['rrze_qr_color']) && $GLOBALS['options']['rrze_qr_color_legacy_migrated'] === '1', 'Migration must remove the old setting and mark completion');
$GLOBALS['options']['rrze_qr_foreground'] = '#123456';
$migrate();
check($settings->getDefaults()['foreground'] === '#123456', 'Repeated migration must not overwrite newer settings');
$GLOBALS['options'] = ['rrze_qr_defaults' => ['foreground' => '#123456', 'background' => '#abcdef', 'size' => 777]];
check($settings->getDefaults() === $GLOBALS['options']['rrze_qr_defaults'], 'Each color and the custom size must be restored independently');
$GLOBALS['options']['rrze_qr_defaults']['background'] = '#123456';
check($settings->getDefaults() === ['foreground' => '#000000', 'background' => '#ffffff', 'size' => 777], 'Identical stored colors must fall back without losing the size');
echo "PHP extracted settings and migration checks passed.\n";
